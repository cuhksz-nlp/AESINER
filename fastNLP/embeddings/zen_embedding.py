...
from ..modules.encoder.bert import _WordPieceBertModel, BertModel
from fastNLP.ZEN import ZenModel, ZenConfig, BertTokenizer, ZenNgramDict, convert_examples_to_features


class ZenEmbedding(ContextualEmbedding):
    """
    ...
    def __init__(self, vocab: Vocabulary, model_dir_or_name: str = 'en-base-uncased', layers: str = '-1',
                 pool_method: str = 'first', word_dropout=0, dropout=0, include_cls_sep: bool = False, pooled_cls=True,
                 requires_grad: bool = True, auto_truncate: bool = False):
        """
        ...
        """
        super(ZenEmbedding, self).__init__(vocab, word_dropout=word_dropout, dropout=dropout)
        ...
        self.model = _WordZenModel(model_dir_or_name=model_dir_or_name, vocab=vocab, layers=layers,
...
class ZenWordPieceEncoder(nn.Module):
    """
...
class _WordZenModel(nn.Module):
    def __init__(self, model_dir_or_name: str, vocab: Vocabulary, layers: str = '-1', pool_method: str = 'first',
        ...
        self.encoder = ZenModel.from_pretrained(model_dir_or_name)
        self.ngram_dict = ZenNgramDict(model_dir_or_name, tokenizer=self.tokenizer)
        ...
        for token in word_piece_dict.keys():
            if token in self.tokenizer.vocab:
                embed.weight.data[len(new_word_piece_vocab)] = original_embed[self.tokenizer.vocab[token]]
            else:
                embed.weight.data[len(new_word_piece_vocab)] = original_embed[self.tokenizer.vocab['[UNK]']]
    ...
    def forward(self, words):
        ...
        # all_outputs: [batch_size x max_len x hidden_size, batch_size x max_len x hidden_size, ...]
            ...
            ngram_ids, ngram_position = convert_examples_to_features(
                words, self.vocab, max_word_len, self.tokenizer, self.ngram_dict)

            print(ngram_ids.shape)
            print(ngram_position.shape)

            ngram_ids[:, 0].fill_(self._cls_index)
            ngram_ids[batch_indexes, word_pieces_lengths + 1] = self._sep_index

        ngram_ids.to(words.device)
        ngram_position.to(words.device)

        bert_outputs, pooled_cls = self.encoder(word_pieces,
                                                input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_position,
                                                token_type_ids=token_type_ids, attention_mask=attn_masks,
