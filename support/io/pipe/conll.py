...
class _NERPipe(Pipe):
    ...
    (创建 :class:`support.Vocabulary` 对象，所以在返回的DataBundle中将有两个Vocabulary); (3）将words，target列根据相应的
    Vocabulary转换为index。
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        :param ~support.DataBundle data_bundle: 传入的DataBundle中的DataSet必须包含raw_words和ner两个field，且两个field的内容均为List[str]在传入DataBundle基础上原位修改。
        :return DataBundle:
        """
        # 转换tag
...
class Conll2003NERPipe(_NERPipe):
    ...
    def process_from_file(self, paths) -> DataBundle:
        ...
        :param paths: 支持路径类型参见 :class:`support.io.loader.ConllLoader` 的load函数。
        :return: DataBundle
        """
        # 读取数据
...
class _CNNERPipe(Pipe):
    ...
    (创建 :class:`support.Vocabulary` 对象，所以在返回的DataBundle中将有两个Vocabulary); (3）将chars，target列根据相应的
    Vocabulary转换为index。
