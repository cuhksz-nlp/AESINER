...
class Loader:
    ...
    - download() 函数：自动将该数据集下载到缓存地址，默认缓存地址为~/.support/datasets/。由于版权等原因，不是所有的Loader都实现了该方法。该方法会返回下载后文件所处的缓存地址。
    - _load() 函数：从一个数据文件中读取数据，返回一个 :class:`~support.DataSet` 。返回的DataSet的内容可以通过每个Loader的文档判断出。
    ...
    def load(self, paths: Union[str, Dict[str, str]] = None) -> DataBundle:
        r"""
        从指定一个或多个路径中的文件中读取数据，返回 :class:`~support.io.DataBundle` 。
        ...
        :return: 返回的 :class:`~support.io.DataBundle`
        """
        if paths is None:
            paths = self.download()
