...
def cached_path(url_or_filename: str, cache_dir: str = None, name=None) -> Path:
    ...
    1. 如果cache_dir=None, 则cache_dir=~/.support/; 否则cache_dir=cache_dir
    ...
    :param str cache_dir: 文件的缓存文件夹。如果为None，将使用"~/.support"这个默认路径
...
def get_cache_path():
    ...
    fastnlp_cache_dir = os.path.expanduser(os.path.join("~", ".support"))
    return fastnlp_cache_dir
...
def get_from_cache(url: str, cache_dir: Path = None) -> Path:
    ...
    # response = requests.head(url, headers={"User-Agent": "support"})
    ...
    if not cache_path.exists():
        ...
        # GET file object
        req = requests.get(url, stream=True, headers={"User-Agent": "support"})
