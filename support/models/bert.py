...
fastNLP提供了BERT应用到五个下游任务的模型代码，可以直接调用。这五个任务分别为
    ...
    - 文本分类任务： :class:`~support.models.BertForSequenceClassification`
    - Matching任务： :class:`~support.models.BertForSentenceMatching`
    - 多选任务： :class:`~support.models.BertForMultipleChoice`
    - 序列标注任务： :class:`~support.models.BertForTokenClassification`
    - 抽取式QA任务： :class:`~support.models.BertForQuestionAnswering`

每一个模型必须要传入一个名字为 `embed` 的 :class:`support.embeddings.BertEmbedding` ，这个参数包含了
:class:`support.modules.encoder.BertModel` ，是下游模型的编码器(encoder)。
...
class BertForSequenceClassification(BaseModel):
    ...
    def forward(self, words):
        ...
        :return: { :attr:`support.Const.OUTPUT` : logits}: torch.Tensor [batch_size, num_labels]
        """
    ...
    def predict(self, words):
        """
        :param torch.LongTensor words: [batch_size, seq_len]
        :return: { :attr:`support.Const.OUTPUT` : logits}: torch.LongTensor [batch_size]
        """
...
class BertForMultipleChoice(BaseModel):
    ...
    def forward(self, words):
        ...
        :return: { :attr:`support.Const.OUTPUT` : logits}: torch.LongTensor [batch_size, num_choices]
        """
...
class BertForTokenClassification(BaseModel):
    ...
    def forward(self, words):
        """
        :param torch.LongTensor words: [batch_size, seq_len]
        :return: { :attr:`support.Const.OUTPUT` : logits}: torch.Tensor [batch_size, seq_len, num_labels]
        """
    ...
    def predict(self, words):
        """
        :param torch.LongTensor words: [batch_size, seq_len]
        :return: { :attr:`support.Const.OUTPUT` : logits}: torch.LongTensor [batch_size, seq_len]
        """
        logits = self.forward(words)[Const.OUTPUT]
        return {Const.OUTPUT: torch.argmax(logits, dim=-1)}
